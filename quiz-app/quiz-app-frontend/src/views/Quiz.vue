<template>
  <div v-if="question" class="min-h-screen flex items-center justify-center bg-brand-grayDark px-4">
    <div class="flex flex-col items-center w-full md:w-2/3 lg:w-1/2">
      ...
      <div class="bg-brand-black rounded-2xl shadow p-6 w-full text-center mb-6">
        ...
        <div class="w-full bg-gray-700 rounded-full h-2 md:h-3 mb-1">
          <div
            class="bg-brand-violet h-2 md:h-3 rounded-full transition-all"
            :style="{ width: progress + '%' }"
          ></div>
        </div>
        <p class="text-right text-xs mt-1 text-gray-400">Zeit übrig: {{ timeLeft }} s</p>
      </div>

      <!-- Antworten -->
      <div class="bg-brand-black rounded-2xl shadow p-6 w-full">
        <ul class="space-y-3 md:space-y-4">
          <li v-for="(opt, i) in question.options" :key="i">
            <button
              @click="select(i)"
              class="w-full text-left px-4 py-3 md:py-4 bg-brand-grayDark border border-gray-700 rounded-lg hover:bg-brand-violet text-base md:text-lg text-white"
            >
              {{ opt }}
            </button>
          </li>
  ...
  <div v-else class="min-h-screen flex items-center justify-center bg-brand-grayDark">
    <p class="text-gray-300">Keine Fragen (mehr) vorhanden.</p>
  </div>
</template>
